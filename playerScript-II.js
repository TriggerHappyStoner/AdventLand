...
sAInvLoopInt 					= 600000
sAHealLoopInt 					= 1000
sAHealAoELoopInt 				= 1000
sAFolLoopInt 					= 1000
sAAttLoopInt 					= 1000
sAAstLoopInt 					= 1000
sTkMdLoopInt 					= 1000
mainLoop 						= setInterval(mainLoop,mainLoopInt)
s1Loop 							= setInterval(s1Loop,s1LoopInt)
sAInvLoop 						= setInterval(sAInvLoop,s1LoopInt)
sAHealLoop 						= setInterval(sAHealLoop,sAHealLoopInt)
sAHealAoELoop 					= setInterval(sAHealAoELoop,sAHealAoELoopInt)
sAFolLoop 						= setInterval(sAFolLoop,sAFolLoopInt)
sAAttLoop 						= setInterval(sAAttLoop,sAAttLoopInt)
sAAstLoop 						= setInterval(sAAstLoop,sAAstLoopInt)
sTkMdLoop 						= setInterval(sTkMdLoop,sTkMdLoopInt)
...
function sendSlotInvtoOtherChars(character){
...
setInterval(function(){
    if(character.name === group[0]){ // You are the leader.
        for(let i = 1; i < group.length; i++)
            if(!parent.party_list.includes(group[i]))
                send_party_invite(group[i]);
    }else{ // You are a follower.
        if(character.party && character.party !== group[0])
